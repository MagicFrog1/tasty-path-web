  ...
  const handleSelectPlan = async (planId: 'weekly' | 'monthly' | 'annual' | 'free') => {
    ...
    if (!stripeAvailable) {
      ...
      console.warn('🔍 Verificando variables de entorno directas:', {
        ...
        NEXT_PUBLIC_STRIPE_PRICE_WEEKLY: env.NEXT_PUBLIC_STRIPE_PRICE_WEEKLY || 'NO ENCONTRADO',
          ...
          return (
            <PlanCard key={plan.id} highlight={highlight} isCurrent={isCurrent}>
              ...
              <SelectButton 
                highlight={highlight} 
                isCurrent={isCurrent}
                onClick={() => !isCurrent && plan.id !== 'free' && handleSelectPlan(plan.id as 'weekly' | 'monthly' | 'annual' | 'free')}
