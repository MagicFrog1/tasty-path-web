...
import { FiCalendar, FiClock, FiTrendingUp, FiChevronRight, FiTrash2, FiPlay, FiEdit, FiCheck, FiX } from 'react-icons/fi';
        ...
        {orderedPlans.map(plan => {
          ...
          return (
            <PlanCard key={plan.id} active={isActive} style={isSelected ? { borderColor: '#2563eb', boxShadow: '0 0 0 2px rgba(37,99,235,0.25)' } : undefined}>
              ...
              <PlanHeader>
                <span>{isActive || plan.status === 'active' ? 'Plan activo' : 'Plan guardado'}</span>
                {isEditing ? (
                  <>
                    <RenameForm
                      onSubmit={event => {
